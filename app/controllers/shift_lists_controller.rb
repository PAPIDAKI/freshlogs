class ShiftListsController < ApplicationController
  ...
  def index
    @shift_lists = ShiftList.where(tenant_id:params[:tenant_id]).order(start_at: :DESC)
  end
  ...
  # GET /shift_lists/new
  def new
    @shift_list = @tenant.shift_lists.new
  ...
  def create
    @shift_list = @tenant.shift_lists.new(shift_list_params)
    @shift_list.tenant_id=params[:tenant_id]


    respond_to do |format|
      if @shift_list.save
        format.html { redirect_to tenant_shift_list_path(@tenant,@shift_list), notice: 'Shift list was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @shift_list.update(shift_list_params)
        format.html { redirect_to tenant_shift_list_path(@tenant,@shift_list), notice: 'Shift list was successfully updated.' }
  ...
  def destroy
    @shift_list.destroy
    respond_to do |format|
      format.html { redirect_to tenant_shift_lists_path(@tenant), notice: 'Shift list was successfully destroyed.' }
